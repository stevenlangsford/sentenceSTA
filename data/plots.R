...
responsecount.df <- responses.df%>%group_by(text,item_type,stim_type,canon_status)%>%summarize(count=n())
responsecount.plot <-
    ggplot(responsecount.df,aes(x=text,y=count,color=canon_status))+
    geom_point()+
