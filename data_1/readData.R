...
responses.df <- read.csv("raw/sentenceresponsedata.csv")%>%
    group_by(ppntID) %>%
    arrange(responseTime) %>% #setup to recover reading times
    mutate(deliberationTime = responseTime - lag(responseTime,
                                                 default = responseTime[1]),
           normTime = (deliberationTime - mean(deliberationTime)) /
               sd(deliberationTime) #standardize reading times. Note there are spacer stim fronting question blocks, the continue button on those 'starts the clock' for the first actual stim: first spacer screen gets a deliberation time of 0.
           ) %>%
    ungroup() %>%
    ...
    mutate(response = as.numeric(as.character(response)),
           ...
           questiontext = ifelse(questiontext == "Is this an <em>acceptable</em> English sentence?",
                                 "is_acceptable",
                                 "is_grammatical") #more code-friendly level names.
           )
...
badids <- demographics.df %>%
    filter(age <= 0) %>%
    select(ppntID) %>% mutate(reason = "badage") #clearly not cooperating, probably deliberately :-)
badids <- badids %>%
    rbind(demographics.df %>%
          filter(!str_detect(language,"en")) %>%
          select(ppntID)
          %>% mutate(reason = "language")) #Reported non english speaker. Admits misspellings though.
##Super-duper fast:
timetaken.df <- responses.df %>%
    group_by(ppntID) %>%
    summarize(beginTime = min(responseTime),
              endTime = max(responseTime),
              totalTime = (endTime-beginTime)/1000/60)

badids <- badids %>%
    rbind(timetaken.df %>%
          filter(totalTime<4) %>%
          select(ppntID) %>%
          mutate(reason = "superfast")) #Always intended to cut superfast responses, but the actual cut-point here is informed by the histogram of response times, ggplot(timetaken.df,aes(x=totalTime))+geom_histogram(binwidth=1)+geom_vline(aes(xintercept=4)). Hope this is not evil, makes sense to me?

##incomplete:
responsecount <- responses.df %>%
    group_by(ppntID) %>%
    summarize(count=n())
...
badids <- badids %>%
    rbind(responsecount %>%
          ...
          select(ppntID) %>%
          mutate(reason = "incomplete")) #Safe to assume max count is the expected number of trials... right? Better like this, better code in a magic number (currently 62), or best to do either but protect with asserts?
##attention checks
attncheck_goodsentence <- responses.df %>%
    filter(text == "Sarah expected to get a good grade.") %>%
    select(ppntID, response)
...
responses.df <- responses.df %>%
    filter(!(ppntID %in% badids$ppntID))
...
##Responses per item:
reps_per_item <- responses.df %>%
    group_by(text, questiontext) %>%
    summarize(count = n()) %>%
    ungroup()
